...
* 可以将字符流转换成字节流
* 作用:可以指定编码方式
### 3.3.2 InputStreamReader
* 字节流转换成字符流
* 可以指定编码方式
## 3.4 缓冲流
* 作用：提高效率
### 3.4.1 字节缓冲输出流BufferedOutputStream
* 继承OutputStream
### 3.4.2 字节缓冲输入流BufferedInputStream
* 继承InputStream
### 3.4.3 字符缓冲输出流BufferedWriter
### 3.4.4 字符缓冲输入流BufferedReader
## 3.5 Properties类
...
## 3.6 序列化流和反序列化流
### 3.6.1 序列化流ObjectOutputStream
* 将对象以流的形式写到文件中，实现序列化，对象必须实现Serializable(标记型接口)接口
* writeObject方法
### 3.6.2 反序列化流ObjectInputStream
* 将文件对象以流的形式读取出来
* readObject方法
### 3.6.3  静态不属于对象，静态数据不能序列化
### 3.6.4 transient关键字
* 阻止成员变量序列化
### 3.6.5  注意点 
