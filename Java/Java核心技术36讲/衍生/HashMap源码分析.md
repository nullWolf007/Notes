...
```text
hashmap在扩容的时候，会创建新的数组，由于数组table.length的长度发生了改变，因此hash&(table.length-1)得到的值发生了改变。要么保持原有index不变，要么位置index+原有数组长度。由于二进制高位新增了一个1。如00001111变成00011111。因此扩容的时候hashmap存储元素的位置并不稳定。
```
